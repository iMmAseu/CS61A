...
def move_stack(n, start, end):
    ...
    if n == 1:
        ...
        list.remove(start)
        list.remove(end)
        move_stack(n - 1, start, list[0])
        move_stack(1, start, end)
        move_stack(n - 1, list[0], end)

from operator import sub, mul

# lambda f: lambda x: f(f, x) 是一个高阶函数，它接受一个函数 f，并返回一个新的函数 lambda x: f(f, x)。
#
# (lambda f, x: 1 if x == 1 else mul(x, f(f, x-1))) 是递归函数的定义，它被传递给外层的 f。
