    ...
    def report(self):
        ...
        if self.changed():
            ...
            if self.before < self.after:
                return str(self.id) + ': ' + "increased " + f"{self.before}->{self.after}"
            else:
                return str(self.id) + ': ' + "decreased " + f"{self.before}->{self.after}"
...
class BankAccount:
    ...
    def deposit(self, amount):
        ...
        self.balance += amount
        new = self.balance
        self.transactions.append(Transaction(self.id, old, new))
        self.id += 1
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        """
        if amount > self.balance:
            t_now = Transaction(self.id, self.balance, self.balance)
            self.transactions.append(t_now)
            self.id += 1
            return 'Insufficient funds'
        old = self.balance
        self.balance -= amount
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if not self.coins:
            return [coin]
        smallest = min(self.coins)
        self.coins = remove_one(self.coins, smallest)
        if coin < smallest:
            if smallest in self.coins:
                self.coins[smallest] += 1
            else:
                self.coins[smallest] = 1
            return [coin]
        elif coin > smallest:
            if make_change(coin - smallest, self.coins) is None:
                if self.coins:
                    if smallest in self.others:
                        self.others[smallest] += 1
                    else:
                        self.others[smallest] = 1
                    return self.change(coin)
                else:
                    self.sum += smallest
                    if self.sum in self.coins:
                        self.coins[self.sum] += 1
                    else:
                        self.coins[self.sum] = 1
                    self.sum = 0
                ...
                self.sum += smallest
                return [smallest] + self.change(coin - smallest)
        else:
            self.sum += smallest
            if self.sum in self.coins:
                self.coins[self.sum] += 1
