...
def floor_div(args):
    ...
    now = args.rest
    while now != nil:
        if isinstance(now.first, int):
            res = res // now.first
        else:
            res = res // calc_eval(now.first)
        now = now.rest
    return res
...
def eval_define(expressions):
    ...
    if isinstance(val.first, int):
        bindings[symbol] = val.first
    else:
        if val.first in bindings:
            bindings[symbol] = bindings[val.first]
        else:
            bindings[symbol] = val.first
    return symbol
