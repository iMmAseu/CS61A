...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        func_symbol = signature.first
        val_symbol = signature.rest
        exp = expressions.rest
        new_exp = Pair(val_symbol, exp)
        env.define(func_symbol, do_lambda_form(new_exp, env))
        return func_symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    now = expressions
    while now.rest != nil:
        if is_scheme_false(scheme_eval(now.first, env)):
            return False
        now = now.rest
    v = scheme_eval(now.first, env, True)
    if is_scheme_true(v):
        return v
    else:
        return False
...
def do_or_form(expressions, env):
    ...
    now = expressions
    while now.rest != nil:
        if is_scheme_true(scheme_eval(now.first, env)):
            return scheme_eval(now.first, env)
        now = now.rest
    return scheme_eval(now.first, env, True)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while now != nil:
        ...
        names = Pair(now.first.first, names)
        vals = Pair(eval_all(now.first.rest, env), vals)
        now = now.rest
...
def do_eval_form(expressions, env):
    return scheme_eval(scheme_eval(expressions.first, env), env)
